...
class RerankService:
        ...
        if not self.model_loaded:
            try:
                ...
                if model_name:
                    ...
                    logger.info(
                        "Rerank model not configured (using None). Reranking disabled."
                    )
                    self.model = None
                ...
                logger.warning(
                    "sentence-transformers not installed. Reranking will be skipped."
                )
                self.model = None
    ...
    def rerank(
        ...
        try:
            ...
            logger.info(
                f"Reranking complete. Top score: {doc_score_pairs[0][1] if doc_score_pairs else 0}"
            )
